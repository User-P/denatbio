(function($){
    ...
    table=$('#reports_table').DataTable( {
        ...
        "bSort" : false,
          ...
          "columns": [
            ...
            {data:"contracts",sortable:false,orderable:false},
    ...
    var user_contracts=$('#user_contracts').val();

    if(user_contracts!=null)
    {
        var user_contracts= JSON.parse(user_contracts);

        var contracts=[];

        user_contracts.forEach(function(contract){
            contracts.push(parseInt(contract.contract_id));
        });

        $('#contracts_assign').val(contracts).trigger('change');
    }
