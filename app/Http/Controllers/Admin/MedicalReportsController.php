...
class MedicalReportsController extends Controller
{
    ...
    public function update(Request $request, $id)
    {
        ...
        //upload photos
        if($request->hasFile('photo1_test'))
        {
            // 
            $file1 = $request->photo1_test;
            $fileName1 = time().$file1->getClientOriginalName();
            $file1->move('uploads/tests',$fileName1);
            GroupTest::where('id',$id)->update([
                'photo1'=>$fileName1
             ]);
        }
        if($request->hasFile('photo2_test'))
        {
            // 
            $file2 = $request->photo2_test;
            $fileName2 = time().$file2->getClientOriginalName();
            $file2->move('uploads/tests',$fileName2);
            GroupTest::where('id',$id)->update([
                'photo2'=>$fileName2
             ]);
        }
